...
public class RollershutterIntentHandler extends IntentHandler {
    ...
    public RollershutterIntentHandler(OpenHabClient openHabClient, ItemRepository itemDao) {
        ...
        if (item != null) {
            ...
            if (!openHabClient.sendCommand(item.getOpenHabItem(), actionStr)) {
                return errorResponse(
                        String.format("I could not operate the shutter at location %s. There was a openHab communication error.",
                                location.getValue()));
            }
        } else {
