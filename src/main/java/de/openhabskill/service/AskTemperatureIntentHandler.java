...
public class AskTemperatureIntentHandler extends IntentHandler {
    ...
    public AskTemperatureIntentHandler(OpenHabClient openHabClient, ItemRepository itemDao) {
    ...
    public SpeechletResponse handleIntentInternal(Intent intent) {
        ...
        final Item item = itemRepository.findByLocationAndItemNameAndItemType(location.getValue(), ITEM_NAME, ItemType.NUMBER);
        ...
        if (item != null) {
            ...
            try {
                ...
                return errorResponse(String.format("I could not get the temperature value for %s", location.getValue()));
            }

            return buildSpeechletResponse(String.format("The %s temperature is %.1f degrees.", location.getValue(), temp), true);
        } else {
